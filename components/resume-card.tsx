...
export default function ResumeCard({ resume }: ResumeCardProps) {
  ...
  const cleanReasoning = useMemo(() => {
    const sanitizedHtml = DOMPurify.sanitize(resume.reasoning || "");
    ...
    listItems.forEach((li) => {
      li.setAttribute(
        "style",
        "margin-bottom: 0.5rem; font-size: 0.875rem; line-height: 1.625;",
      );
    });
  ...
  return (
    <Card className="h-full grid grid-rows-[1fr_auto] shadow-sm hover:shadow-md transition-shadow cursor-pointer">
      ...
      >
        <CardHeader className="pb-2">
          <CardTitle className="flex justify-between items-center">
            ...
            <span
              className={`px-3 py-1 rounded-full text-sm font-medium ${resume.qualified ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700"}`}
            >
        ...
        <CardContent className="space-y-3">
          <div className="flex items-center justify-between">
            ...
            <span
              className={`px-3 py-1 rounded-full font-semibold ${scoreColor}`}
            >
              {resume.score}
            </span>
          </div>
